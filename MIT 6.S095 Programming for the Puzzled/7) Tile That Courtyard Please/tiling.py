...
def validYardFourTile(n, missSquaresList):
    size = 2**n
    missingQuads = []
    count = 0
    
    for missTile in missSquaresList:
        rMiss = missTile[0]
        cMiss = missTile[1]
        missingQuads.append(2*(rMiss >= size//2) + (cMiss >= size//2))
        
    for i in range(4):
        if i in missingQuads:
            count += 1
    
    #Test for four different quadrants
    if count == 4:
        return True
    
    #Test for tromino arangement
    for (r, c) in missSquaresList:
        if (((r+1, c) and (r, c+1) in missSquaresList) or\
            ((r, c+1) and (r-1, c) in missSquaresList) or\
            ((r, c-1) and (r-1, c) in missSquaresList) or\
            ((r-1, c) and (r+1, c) in missSquaresList)):
               return True
...
print(validYardFourTile(3, [(4, 4), (1, 1), (2, 1), (1, 2)]))
print(validYardFourTile(3, [(4, 4), (3, 1), (2, 1), (1, 2)]))
print(validYardFourTile(3, [(3, 7), (4, 4), (4, 6), (4, 7)]))